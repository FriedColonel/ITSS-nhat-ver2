const APP_ID = 'f1e82df6b23544d98adc09907d1f36f7';
...
let selectedTopic = sessionStorage.getItem('selectedTopics')
...
let joinRoomInit = async () => {
  ...
  rooms.forEach((room) => {
    ...
    if (roomData.members.length >= 2) return;
  ...
  if (similarRooms.length) {
    let index = Math.floor(Math.random() * similarRooms.length);

    roomId = similarRooms[index].id;
    ...
    await db
      ...
      .update({
        members: [
          ...similarRooms[index].data().members,
          {
            uid,
            displayName,
            selectedTopic,
...
document.getElementById('lobby-btn').addEventListener('click', () => {
  if (!rtmClient) window.location = 'index.html';
  else
    db.collection('rooms')
      .doc(roomId)
      .get()
      .then(async (doc) => {
        ...
        if (members.length === 2) {
          ...
          await db.collection('rooms').doc(uid).delete();
        }
      })
